...
test( 'Check Plugin Setting Page', async ( { page } ) => {
    ...
    await expect(page.getByRole('cell', { name: 'Choose which user roles' }).getByLabel('Subscriber')).toBeChecked();

    await page.locator('input[name="cmt_mntn_email_enable"]').check();
    await expect(page.locator('input[name="cmt_mntn_email_enable"]')).toBeChecked();
